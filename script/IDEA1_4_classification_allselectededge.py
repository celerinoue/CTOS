...
def data_load():
    df_all = []
    for i in sorted(glob.glob("result/txt/IDEA1_4/ClassificationDataSet/*.txt")):
...
def plot(all_accuracy):
    ...
    for i in range(len(y)):
        ...
        ax.set_ylim([0, 1])
        ax.set_title(
            f'Accuracy of Binary Classification [Data = {node[gitnum_node[i]]}, CorrCoef(th = {th[i]})]')
        ax.set_xlabel(f'drug name')
        ax.set_xticklabels(drug)
        ax.set_ylabel('Accuracy')

        savepath = f'result/fig/IDEA1_4/Classification_result/fig_Classification_result_{node[num_node[i]]}_th{th[i]}_.png'
