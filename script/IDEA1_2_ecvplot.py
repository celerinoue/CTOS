...
def data_load():
    ...
    return data_ecv_, list_gr_, data_drug_name


# reshape
def reshape(data_ecv_, list_gr_):
    # reshape data_ecv
    ...
    return data_ecv, list_gr_d28


def corrplot(data_ecv, data_drug_name, list_gr_d28):
    list_corr_ = []
...
def scatterplot_maxcorrpair(data_drug_name, list_corr, data_ecv, list_gr_d28):
    for d in range(len(data_drug_name['drug_name'])):
        ...
        # save
        savepath = f'./result/fig/IDEA1_2/fig_scatterplot_{data_drug_name["drug_name"][d]}.png'
        plt.savefig(savepath, dpi=300, format='png', bbox_inches="tight")
        print(f'[SAVE]: {savepath}')
    return
...
if __name__ == '__main__':
    ...
    # reshape the data
    data_ecv, list_gr_d28 = reshape(data_ecv_, list_gr_)
    # corr plot
